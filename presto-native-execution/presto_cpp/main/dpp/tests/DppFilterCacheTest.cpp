...
TEST_F(DppFilterCacheTest, capEnforced) {
  DppFilterCache::initialize(1ULL << 20);
  auto* cache = DppFilterCache::getInstance();
  ASSERT_NE(cache, nullptr);

  auto pool = cache->createTaskPool("test.task.cap");

  std::vector<void*> buffers;
  // Each allocation is half the cap. The third should not fit; the pool's
  // root cap is enforced via the arbitrator and the allocation throws.
  buffers.push_back(pool->allocate(256 * 1024));
  buffers.push_back(pool->allocate(256 * 1024));

  bool threw = false;
  try {
    buffers.push_back(pool->allocate(1024 * 1024));
